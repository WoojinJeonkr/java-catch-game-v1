...
public class Monster6 extends MonsterBase{
	...
	public boolean catchMonster() {
		...
		this.e = r.nextInt(3);
		if(e == 0) {
			System.err.println("\"60% 확률로 그란돈이 포켓볼에서 빠져나갔다! 대지의 힘으로 벗어나버렸다!\"\n" +
					"➤ \"그라라라라~ (너희들이 날 막을 수 있다고 생각했어?)\"\n" +
					"(그란돈은 포켓볼에서 힘껏 튕겨나가며 바다를 흔든다. 썩어도 준치라고 만만치 않은 상대이다.)");
		} else if (e == 1) {
			System.err.println("\"60% 확률로 그란돈은 잡히지 않았다! 땅(?)의 힘이 강력해서 포켓볼을 흔들어버렸다!\"\n" +
					"➤ \"그라~돈! (이런 작은 장난감에 내가 갇힐 수 없다!)\"\n" +
					"(그란돈이 포켓볼 안에서 바다를 흔들어버리며 다시 탈출한다. '잡히지 않는 자'의 위엄!)");
		} else {
			System.err.println("\"60% 확률로 그란돈은 여전히 포켓볼을 뚫고 나온다! 대지의 신이 이 작은 장난감에 갇힐 순 없다!\"\n" +
